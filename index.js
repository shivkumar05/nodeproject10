...
//============================[OnGoing Drills]=======================================
app.use('/image', express.static('./upload/videos'))
app.get("/:userId/OnGoingDrills", commnMid.jwtValidation, commnMid.authorization, upload.array("videos", 100), async (req, res) => {
    try {
        let data = req.body;
        let file = req.files;
        ...
        let Allrecommendations = await recommendationModel.find().select({ _id: 0, createdAt: 0, updatedAt: 0, __v: 0 });
...
//===========================[ post Recommendation] =====================
app.use('/image', express.static('./upload/videos'))
app.post("/:userId/recommendation", commnMid.jwtValidation, commnMid.authorization, upload.array("audioFile", 100), async (req, res) => {
    try {
        let data = req.body;
        let file = req.files;
        ...
        let obj = {}
        obj["anecdote_no"] = RecommendationCreated.anecdote_no
        obj["mesage"] = RecommendationCreated.mesage
        obj["audioFile"] = RecommendationCreated.audioFile
        obj["audiolength"] = RecommendationCreated.audiolength
        obj["createdat"] = RecommendationCreated.createdat
        obj["manual"] = RecommendationCreated.manual
